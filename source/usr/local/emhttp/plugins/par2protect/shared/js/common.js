...
(function(P) {
        ...
        trigger: function(event, data) {
            ...
            this.listeners[event].forEach(callback => {
                try {
                    ...
                    P.logger.error('Error in event listener:', { error: e });
                }
            });
        }
    };
    ...
    P.triggerOperationCompleted = function(operationType, status, result) {
        // ('Triggering operation completed event for direct operation:', {
        //     type: operationType,
        //     status: status,
        //     result: result
        // });
        ...
        // Also directly refresh the list if it's currently loaded
        if (P.list && typeof P.list.refreshProtectedList === 'function') {
            P.logger.debug('Directly refreshing protected files list');
            // Refresh immediately first with loading indicator
            P.list.refreshProtectedList(true);
            
            // Then use a small delay to ensure the server has updated its data
            setTimeout(function() {
                P.logger.debug('Refreshing protected files list again after delay');
                P.list.refreshProtectedList(true);
            }, 1000);
        }
    };
            ...
            success: function(response) {
                if (response.success && response.data) {
                    ...
                    if (response.data.file_types &&
                        ...
                        P.logger.debug('Loaded custom file extensions:', {
                    ...
                    if (P.logger && typeof P.logger.initWithSettings === 'function') {
                        ...
                        // P.logger.info('Logger initialized with server settings');
                    }
                }
            },
